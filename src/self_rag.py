...
def grade_relevance(state : RAGState):
    ...
    return {
        ...
        "trace" : _trace(state, f"Relevance grade ({mode}): {len(relevant)}/{len(state.get("docs", []))} relevant")
    }
...
def rewrite_internal_query(state : RAGState):
    ...
    return {
        "retrival_query": out.query,
        "retrival_rewrites": state.get("retrieval_rewrites", 0) + 1, 
        "docs": [],
        "relevant_docs": [],
...
def web_search(state : RAGState):
    ...
    return {
        ...
        "source_mode": "web",
        "used_web_search": True,
        "trace": _trace(state, f"Internet search: {len(docs)} results") 
    }
...
def check_support(state : RAGState):
    ...
    return {
        ...
        "trace": _trace(state, f"Support check: {out.status}")
    }
...
def revise_answer(state : RAGState):
    ...
    return {
        ...
        "trace": _trace(state, "Revised answer for grounding")
    }
...
def check_usefulness(state : RAGState):
    ...
    return {
        ...
        "trace": _trace(state, f"Usefulness check: {out.status}")
    }
...
def no_answer(state : RAGState):
    return {
        "answer": "I could not find enough reliable runbook or external evidence to recommend a safe troubleshooting action.", 
        "source_mode": "none", 
        "trace": _trace(state, "Stopped: no reliable answer found")
    }
